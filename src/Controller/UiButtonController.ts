...
class Button {
    ...
    public constructor(buttonType: ButtonType, text: string, title: string, actionName: string = "", largeButton: boolean = false) {
    ...
    private getOptionByButtonType(): boolean {
        switch(this.buttonType) {
            case ButtonType.ZoomIn: return options.hideZoomInButton;
            case ButtonType.ZoomOut: return options.hideZoomOutButton;
            case ButtonType.ResetZoom: return options.hideResetZoomButton;
            case ButtonType.FullZoom: return options.hideFullZoomButton;
        }

        throw "ButtonType '" + this.buttonType + "' out of range";
    }
...
class UiButtonController {
    public initButtons(videoController: VideoController): ContainerElement {
        ...
        return new ContainerElement(...buttons.filter(button => button !== undefined));
    }

    private createButton(videoController: VideoController, buttonType: ButtonType, text: string, title: string, actionName: string, largeButton: boolean = false): ButtonElement {
