...
class StatisticController {
    ...
    public static toggle(videoController: VideoController): void {
        if (videoController !== undefined && videoController !== null) {
            if (
                videoController.getHtmlVideo !== undefined &&
        ...
        if (this._statisticParent.classList.contains("hidden")) this.disable()
        else this.enable()
    }
    ...
    private static enable(): void {
        const updateVideoStats = (function (_this) {
            ...
            let prevFrames: number =
                _this._video.getVideoPlaybackQuality().totalVideoFrames
