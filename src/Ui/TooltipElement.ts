...
class TooltipElement extends UiElement {
    ...
    public addChildEventListener(event: string, listener: EventListener): void {
        this._element.childNodes.forEach(child => child.addEventListener(event, listener, false));
    }

    public selectTooltipChild(child: HTMLElement) {
        this._element.childNodes.forEach(element => (element as HTMLElement).classList.remove("tooltipChildSelected"));
